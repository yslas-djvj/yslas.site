...
4. To put it in the menu, add `['NEWPAGE', 'newpage']` to `MENU_LINKS`
   in `js/site.js`.
5. Add a 301 line for it in `_redirects` alongside the others.
...
Every internal link is a clean, root-absolute slug — `/about`, not
`/about.html`. The **files on disk keep their `.html` extension**; only the
links are clean, and the host maps one to the other.

- Links are built in one place: `cleanHref()` in `js/site.js` (§5). It accepts
  `about`, `about.html` or `/about` and always emits `/about`, so you cannot
  break this by writing the wrong form in `MENU_LINKS`.
- **GitHub Pages** and **Netlify** both resolve `/about` → `about.html` with no
  configuration.
- `_redirects` adds Netlify 301s from the old `.html` URLs to the clean ones,
  so a page is never reachable at two addresses. GitHub Pages ignores the file.
- **Opening the files directly (`file://`) will break navigation**, and so will
  a plain `python -m http.server` — neither resolves extensionless paths. Test
  clean URLs on the deployed site (or any server with pretty-URL support).
