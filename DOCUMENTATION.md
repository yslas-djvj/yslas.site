...
1. Push the whole folder to a GitHub repo (or drag into Netlify). This build
   needs no custom domain and no pretty-URL support — links are plain `.html`.
2. Keep `.nojekyll` at the root — without it GitHub Pages runs Jekyll, which
   **excludes every file starting with an underscore**. Three EPK gallery
   photos are named `_51a0222.jpg`, `_51a0238.jpg` and `_51a0303.jpg`, so they
   would vanish from the live site. Check `.nojekyll` survived the upload:
   dotfiles are hidden by most file pickers and drag-and-drop flows.
