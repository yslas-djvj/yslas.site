...
/* ============ MENU — EDIT ONCE, CHANGES ON EVERY PAGE ============
   ...
   in a new tab; anything else is a page of this site and is emitted as a
   RELATIVE .html link — "about" and "about.html" both become href="about.html".
   This is the .html build: it works on any host (and straight off the file
   system) with no pretty-URL support required. See the sibling `yslas/` folder
   for the clean-slug build.
   NOTE: the EPK exists but is deliberately NOT in this menu — it is the
   unlisted press-kit page, reachable only by its direct URL (epk.html). */
const MENU_LINKS = [
	['ABOUT', 'about'],
	['SONICS', 'sonics'],
	['VISUALS', 'visuals'],
...
/* Turn a destination into a RELATIVE .html href. Accepts "about", "about.html"
   or "/about" and always emits "about.html"; "", "/", "index" and "index.html"
   all collapse to "index.html". External URLs pass through untouched.
   Relative (not root-absolute) so the site also works from a subfolder or
